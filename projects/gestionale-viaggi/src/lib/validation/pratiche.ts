...
import { optionalDate, optionalText } from '@/lib/validation/comune'
 ...
 */
const amountCents = (etichetta: string, { obbligatorio = false } = {}) =>
  z
    .string()
    .trim()
    .optional()
...
const percentBps = (etichetta: string, max = 100) =>
  z
    .string()
    .trim()
    .optional()
...
const intero = (etichetta: string, min: number, max: number, predefinito: number) =>
  z
    .string()
    .trim()
    .optional()
